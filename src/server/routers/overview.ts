...
export const overviewRouter = createRouter()
	.query('index', {
		async resolve({ ctx: { user } }) {
			const items =
				(await db.$queryRaw`SELECT TO_CHAR(i."measuredFormat",'dd.MM.YY')AS"date",phase,calories,round("weight"::numeric,2)AS"weight",round("bodyFat"::numeric,2)AS"bodyFat",(ac."workoutDuration" IS NOT NULL)AS"training",ac."workoutDuration",notes,i."id","creatine" FROM"Info" AS i LEFT JOIN(SELECT"userId","measuredFormat",SUM(calories)AS calories FROM"Nutrition" GROUP BY"measuredFormat","userId")n ON i."userId"=n."userId" AND i."measuredFormat"=n."measuredFormat" LEFT JOIN(SELECT"userId","measuredFormat","weight","bodyFat" FROM"Measurements" AS me)m ON i."userId"=m."userId" AND i."measuredFormat"=m."measuredFormat" LEFT JOIN(SELECT"userId","measuredFormat",sum(duration)AS"workoutDuration" FROM"Workouts" GROUP BY"userId","measuredFormat")ac ON i."userId"=ac."userId" AND i."measuredFormat"=ac."measuredFormat" LEFT JOIN(SELECT"userId","measuredFormat",creatine FROM"Supplements")su ON i."userId"=su."userId" AND i."measuredFormat"=su."measuredFormat" WHERE i."userId"=${user?.id} ORDER BY i."measuredFormat" DESC;
            `) as OverviewData[];
			if (user) {
